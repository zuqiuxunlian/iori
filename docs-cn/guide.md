...
按住顶部大按钮不松手，同时再按开机，进入设置模式。操作大按钮，短按切换选项，长按确认。

**菜单分为三部分：**

模式：M.开头

- TAP：起点使用，用于分段计时，多道计时和行进间
- SPT：终点使用，用于分段计时，多道计时和行进间
- MORE：更多模式选择，长按进入子菜单选择
  - SET：自动发令模式。SET 和 GO 之间随机 1.5-3.5 秒发枪。
  - ADD：多人计时模式。每次触发增加一次计时。适合 400、800 多人计时。
  - LAP：单圈计时模式。记录每一圈的成绩。适合轮滑。
  - SHT：多次返跑模式。触发规定次数后再停止。适合 5-10-5 等。
  - KEEP：暂停继续模式。短按暂停/继续，长按停止。适合比赛计时。
  - CNT：计数模式。每次按下或触发记录一次。适合俯卧撑等计数。

设置：S.开头

- TWIN：分组设置。同一小组的设备开机自动连接。不同小组设备互相不干扰。
  - A 至 G，选择分组后，长按确认。
  - OFF，不参与分组，单机使用。
- TRIG：触发设置。行进间起点使用 STRT 或者 BOTH。
  - BOTH 允许触发开始和结束。
  - STRT 仅允许触发开始。
  - STOP（默认）仅允许触发结束。
- DIST：触发距离设置。可选 1.3 米（默认）至 6.3 米。
- DEC：小数点设置。可选 0.01（默认）百分位和 0.001 千分位

其他

- BT...：重设蓝牙连接。连接后，可以实现一键控制手机录像功能。
- ABOUT：计时器的型号和软件版本显示。

::: tip TAP-R 模式，新增独立准备状态（类似清零）
在 M.TAP 菜单下，快速双击可以切换到 TAP-R 模式，长按确认。试试吧!
:::

::: tip 设置菜单差异
以上菜单对应软件版本 3.3.0。不同软件版本，菜单项目可能会略有不同。
:::
